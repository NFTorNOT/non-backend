...
class CommonValidator {
  ...
  static validatePaginationIdentifier(key) {
    try {
      ...
      return CommonValidator.validateNextOrPreviousPagePayload(paginationParams);
    } catch (error) {
      return false;
    }
  }
  ...
  /**
   * Is var a valid transaction hash string?
   *
   * @param {string} variable
   *
   * @returns {boolean}
   */
  static validateTransactionHashString(variable) {
    if (CommonValidator.isVarNullOrUndefined(variable)) {
      return false;
    }

    return /^0x([A-Fa-f0-9]{64})$/.test(variable);
  }
}
